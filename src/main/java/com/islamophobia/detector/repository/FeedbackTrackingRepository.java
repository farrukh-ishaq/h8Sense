...
public interface FeedbackTrackingRepository extends JpaRepository<FeedbackTracking, UUID> {
    ...
    @Modifying
    @Query("UPDATE FeedbackTracking ft SET ft.feedbackCount = ft.feedbackCount + 1, ft.lastFeedbackAt = CURRENT_TIMESTAMP WHERE ft.id = :id")
    void incrementFeedbackCount(@Param("id") UUID id);
}
